...
final class ChartViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let timeLabels = candles.map { $0.timeLabel }
        ...
        lineChartView.setInitialPrices(prices)
        
        setupLineChartSelection()
...
private extension ChartViewController {
    ...
    func setupChartViews() {
        ...
        lineChartView.backgroundColor = UIColor(white: 0.10, alpha: 1)
    ...
    func updateLineChartSelection() {
        // Визуально выделяем выбранную точку на графике
        // Так как ChartView не имеет встроенного выделения,
        // можно добавить дополнительный слой поверх
        lineChartView.setNeedsDisplay()
    }

    func setupLineChartSelection() {
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(handleLineChartTap(_:)))
        lineChartView.addGestureRecognizer(tapGesture)
    }

    @objc func handleLineChartTap(_ gesture: UITapGestureRecognizer) {
        let location = gesture.location(in: lineChartView)
        
        guard candles.count > 1 else { return }
        
        let rect = lineChartView.bounds
        let padding: CGFloat = 20
        let drawWidth = rect.width - padding * 2
        
        let relativeX = location.x - padding
        let index = Int(round((relativeX / drawWidth) * CGFloat(candles.count - 1)))
        
        let clampedIndex = max(0, min(index, candles.count - 1))
        selectEntry(at: clampedIndex, updateCollectionSelection: true, scrollPosition: .centeredHorizontally)
    }
    ...
    func selectEntry(at index: Int, updateCollectionSelection: Bool, scrollPosition: UICollectionView.ScrollPosition) {
        ...
        if chartMode == .line {
            highlightPoint(at: index)
        }
        
        showRecommendation(for: candle)
    }

    func highlightPoint(at index: Int) {
        lineChartView.setNeedsDisplay()
    }
